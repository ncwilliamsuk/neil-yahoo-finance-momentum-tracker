...
export function ScreenerTab() {
  ...
  const [soniaRate, setSoniaRate]       = useState<number | null>(null);
  const [soniaSource, setSoniaSource]   = useState<string>('');
  ...
  // ── Data fetch ───────────────────────────────────────────────
  const fetchData = async (targetUniverse: Universe = universe) => {
    setIsLoading(true);
    setLoadError(null);

    try {
      const res    = await fetch(`/api/v5/etf-screener?universe=${targetUniverse}`);
      const result = await res.json() as ApiResponse;

      if (result.success) {
        setApiData(result.data);
        setSoniaRate(result.soniaRate);
        setSoniaSource(result.soniaSource);
        ...
        console.log(
          `✓ Screener: ${result.successCount} ETFs loaded (${targetUniverse}), ` +
          `SONIA: ${result.soniaRate?.toFixed(2)}% via ${result.soniaSource}`
        );
      } else {
        setLoadError(result.error ?? 'Unknown error from API');
        console.error('Screener API error:', result.error);
      }
    } catch (err) {
  ...
  // ── Handlers ─────────────────────────────────────────────────
  const handleRefresh = () => fetchData(universe);
  ...
  if (isLoading) {
    ...
    // Estimated time: SONIA fetch ~1s + (etfCount × 200ms) + buffer
