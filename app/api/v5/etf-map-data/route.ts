// app/api/v5/etf-map-data/route.ts
import { NextResponse } from 'next/server';
// @ts-ignore
import YahooFinance from 'yahoo-finance2';
import { ALL_ETF_CATEGORY_TICKERS, calcReturn, Timeframe } from '@/lib/v5/sectors';
...
export async function GET() {
  try {
    ...
    for (const ticker of ALL_ETF_CATEGORY_TICKERS) {
      await new Promise(r => setTimeout(r, 200));
      priceMap[ticker] = await fetchPrices(ticker);
    }

    // Build returns for every ticker across all timeframes
    ...
    console.error('[etf-map-data] error:', err?.message ?? err);
    return NextResponse.json({ error: 'Failed to fetch ETF map data' }, { status: 500 });
  }
}
